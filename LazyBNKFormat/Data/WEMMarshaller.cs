...
namespace LazyBNKFormat.Data {
	...
	public class WEMMarshaller {
		...
		/// </summary>
		private WEMFileData[] WEMFilesInternal = null;
		...
		/// <summary>
		/// Every WEM identity within this marshaller. Use <seealso cref="OverwriteWEMFile(WEMFileData)"/> to modify the identity list.
		/// </summary>
		...
		/// <summary>
		/// Every WEM file within this marshaller. Use <seealso cref="OverwriteWEMFile(WEMFileData)"/> to modify the file list.
		/// </summary>
		public IReadOnlyList<WEMFileData> WEMFiles {
			get {
			...
			get {
				if (RawNeedsUpdate) {
					...
					for (int idx = 0; idx < WEMFilesInternal.Length; idx++) {
						...
						WEMFileData file = WEMFilesInternal[idx];
		...
		public WEMMarshaller(WEMFileIdentity[] identities, byte[] condensedWEMFileArray) {
			...
			WEMFilesInternal = new WEMFileData[identities.Length];
			...
			for (int idx = 0; idx < identities.Length; idx++) {
				...
				WEMFilesInternal[idx] = new WEMFileData() {
					ID = identity.WemID,
					Data = data
				};
			}
		...
		/// <returns></returns>
		public WEMFileData GetWemFile(WEMFileIdentity identity) {
		...
		/// <returns></returns>
		public WEMFileData GetWemFile(uint identity) {
			foreach (WEMFileData file in WEMFilesInternal) {
				if (file.ID == identity) {
					return file;
				}
			}
		...
		public void OverwriteWEMFile(WEMFileData newFile) {
			for (int idx = 0; idx < WEMFilesInternal.Length; idx++) {
				WEMFileData file = WEMFilesInternal[idx];
