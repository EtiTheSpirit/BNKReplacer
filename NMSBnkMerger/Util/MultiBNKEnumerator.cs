...
namespace NMSBnkMerger.Util {
	...
	public class MultiBNKEnumerator {
		...
		public void PatchEverything() {
			...
			List<WEMFileData> conflictingWEMs = new List<WEMFileData>();
			List<string> conflictingPaths = new List<string>();
			List<WEMFileData> vanillaWEMFiles = VanillaFile.Marshaller.WEMFiles.ToList();
			int currentFiles = 0;
			...
			for (int idx = 0; idx < vanillaWEMFiles.Count; idx++) {
				...
				WEMFileData vanillaWEM = vanillaWEMFiles[idx];
				for (int oidx = 0; oidx < Patches.Length; oidx++) {
					BNKFile otherBnk = Patches[oidx];
					WEMFileData other = GetWEMInOtherBNKFromID(vanillaWEM, otherBnk);
		...
		/// <returns></returns>
		private WEMFileData GetWEMInOtherBNKFromID(WEMFileData template, BNKFile otherArchive) {
			List<WEMFileData> otherWemFiles = otherArchive.Marshaller.WEMFiles.ToList();
			for (int idx = 0; idx < otherWemFiles.Count; idx++) {
				WEMFileData wemFile = otherWemFiles[idx];
